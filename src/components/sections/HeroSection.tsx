      ...
      <div className="container-page relative z-10 grid grid-cols-1 items-center gap-12 py-14 md:py-20 lg:grid-cols-[minmax(0,618px)_minmax(0,1fr)] lg:gap-10">
        ...
        <div className="flex flex-col gap-12 md:gap-14">
          ...
          <div className="flex flex-col gap-7">
            ...
            {/* Gast Speaker badges — compact so all three fit on a single
                line in the ~540px hero column on desktop. Stacks on mobile. */}
            <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-2.5">
              {/* Noah's stage shot is too wide to read at avatar size, so the
                  badge uses a head-and-shoulders crop of it. */}
              {[
                ...
                <a
                  key={s.name}
                  href="#gast-speaker"
                  className="group inline-flex w-fit items-center gap-2.5 rounded-full border border-purple-2/40 bg-white/[0.03] py-1.5 pl-1.5 pr-4 transition-colors duration-300 hover:border-purple-2/70 hover:bg-white/[0.06]"
                >
                  <span className="relative block size-9 shrink-0 overflow-hidden rounded-full border border-white/15">
                    <Image
                      src={s.image}
                      alt={s.name}
                      fill
                  ...
                  <span className="flex flex-col leading-tight">
                    <span className="font-body text-[10px] italic tracking-[0.2px] text-white/55">
                      Gast Speaker
                    </span>
                    <span className="whitespace-nowrap font-display text-[14px] leading-tight tracking-[-0.2px] text-white">
                      {s.name}
                    </span>
                  </span>
                </a>
